...
int
network_set_neuron_number (network * nn, unsigned long nr)
{
  int i;
...
int
network_set_layer_number (network * nn, unsigned long nr)
{
  if (!nn || !nr)
    return -1;
...
int
network_neuron_set_connection_number (neuron * ne, unsigned int nr)
{
  int i;
  if (!ne || !nr)
    return -1;
...
__network_check (network * nn)
{
  ...
  for (i = 0; i < nn->num_of_layers - 1; ++i)
    if ((nn->layers[i].neurons + nn->layers[i].num_of_neurons) !=
        nn->layers[i + 1].neurons)
      {
        printf
          ("Error: There is layers error setting between layer %d and %d\n",
           i, i + 1);
        return -1;
      }
          ...
          for (j = 0; j < nn->layers[i].num_of_neurons; ++j)
            {
              printf ("    ");
              printf ("n[%d] (%d inputs)\n",
                      nn->layers[i].neurons[j].global_id,
                      nn->layers[i].neurons[j].num_input);
              for (k = 0; k < nn->layers[i].neurons[j].num_input; ++k)
                printf ("\tn[%d] (%.2f)",
...
nnet_print (struct nnet *net)
{
  ...
  if (net->outputcount == 1)
    printf ("node %d is the output node.\n", net->nodecount - 1);
  else
    printf ("nodes %d through %d are output\n",
  ...
  for (unsigned int concount = 0; concount < net->synapsecount; concount++)
    printf ("%5d || %3d-->%-3d || %6.2f\n", concount, net->sources[concount],
            net->dests[concount], net->weights[concount]);
  printf ("\n============\n");
}
...
network_run_algorithm (network * nn, network_config * config)
{
  ...
  [SIMULATED_ANNEALING] = simulated_annealing,
      ...
      [GENETIC_ALGORITHM] = genetic_algorithm,[MSMCO] = msmco,};
...
void
network_config_free (network_config * config)
{
  if (!config)
    return;
...
void
network_config_set_default (network_config * config)
{
...
nnet_alloc_default ()
{
  ...
  if (retval == NULL)
    {
      printf ("Unable to allocate (in nnet_alloc_default).\n");
      exit (1);
    }
...
void
SwapRange (struct nnet *net, int start, int star2, int len)
{
  int count;
  int swap;
  unsigned int wsap;
  if (start > star2)
    SwapRange (net, star2, start, len);
  if (net == NULL || len < 0 || start < 0 || star2 < 0
      || star2 + len >= net->nodecount)
    {
      printf ("improper call to SwapRange.\n");
      exit (1);
    }
  if (start + len >= star2)
    {
      printf ("SwapRange cannot swap overlapping ranges.\n");
      exit (1);
    }
  if (start < net->inputcount && star2 + len >= net->inputcount)
    {
      printf ("SwapRange cannot swap between input and non-input nodes.\n");
      exit (1);
    }
  if (start < (net->nodecount - net->outputcount)
      && star2 + len >= (net->nodecount - net->outputcount))
    {
  ...
  for (count = 0; count < len; count++)
    {
      swap = net->transfer[start + count];
      net->transfer[start + count] = net->transfer[star2 + count];
      net->transfer[star2 + count] = swap;
      swap = net->accum[start + count];
      net->accum[start + count] = net->accum[star2 + count];
      net->accum[star2 + count] = swap;
      wsap = net->transferwidths[start + count];
      net->transferwidths[start + count] = net->transferwidths[star2 + count];
      net->transferwidths[star2 + count] = wsap;
    }
  for (count = 0; count < net->synapsecount; count++)
    {
      if (net->sources[count] > start && net->sources[count] < start + len)
        net->sources[count] += (star2 - start);
      else if (net->sources[count] > star2
               && net->sources[count] < star2 + len)
        net->sources[count] += (start - star2);
      if (net->dests[count] > start && net->dests[count] < start + len)
        net->dests[count] += (star2 - start);
      else if (net->dests[count] > star2 && net->dests[count] < star2 + len)
        net->dests[count] += (start - star2);
    }
}
...
int
InsertNodes (struct nnet *net, int startloc, int newcount, int transferfn,
             int accumfn, unsigned int xfersize)
{
  if (net == NULL || newcount < 0)
    {
      fprintf (stderr, "Program Error: improper call to InsertNodes.\n");
      exit (1);
    }
  if (net->nodecount == 0)
    {
  ...
  if (net->transfer != NULL && net->accum != NULL
    ...
    for (count = 1; count < startloc; count++)
      {
        newtrans[count] = net->transfer[count];
        newacc[count] = net->accum[count];
        xwidth[count] = net->transferwidths[count];
      }
  for (count = startloc; count < startloc + newcount; count++)
    {
      newtrans[count] = transferfn;
      newacc[count] = accumfn;
      xwidth[count] = xfersize;
    }
  if (net->transfer != NULL && net->accum != NULL
      && net->transferwidths != NULL)
    for (count = startloc; count < net->nodecount; count++)
      {
        newtrans[count + newcount] = net->transfer[count];
        newacc[count + newcount] = net->accum[count];
        xwidth[count + newcount] = net->transferwidths[count];
      }
  free (net->transfer);
  free (net->accum);
  free (net->transferwidths);
  net->transfer = newtrans;
  net->accum = newacc;
  net->transferwidths = xwidth;
  net->nodecount += newcount;
  for (count = 0; count < net->synapsecount; count++)
    {
...
int
AddHiddenNodes (struct nnet *net, int newnodes, int transferfn, int accumfn,
                unsigned int xfersize)
{
  return (InsertNodes
          (net, net->nodecount - net->outputcount, newnodes, transferfn,
           accumfn, xfersize));
}
...
int
AddInputNodes (struct nnet *net, int newnodes, int transferfn, int accumfn,
               unsigned int xfersize)
{
  net->inputcount += newnodes;
  return (InsertNodes
          (net, net->inputcount - newnodes, newnodes, transferfn, accumfn,
           xfersize));
}
...
int
AddOutputNodes (struct nnet *net, int newnodes, int transferfn, int accumfn,
                unsigned int xfersize)
{
  net->outputcount += newnodes;
  return (InsertNodes
          (net, net->nodecount, newnodes, transferfn, accumfn, xfersize));
}

// inserts connections into an existing network.
void
AddConnections (struct nnet *net, int fromstart, int fromend, int tostart,
                int toend, flotype * weight)
{
  int wt = 0;
  int newcount =
    net->synapsecount + (1 + fromend - fromstart) * (1 + toend - tostart);
  net->weights =
    (flotype *) realloc (net->weights, newcount * sizeof (flotype));
  net->sources =
    (unsigned int *) realloc (net->sources, newcount * sizeof (int));
  net->dests = (unsigned int *) realloc (net->dests, newcount * sizeof (int));
  if (net->weights == NULL || net->sources == NULL || net->dests == NULL)
    {
      printf ("Runtime error: allocation failure in AddConnections.\n");
      exit (1);
    }
                          ...
                          int tostart, int toend)
{
  ...
  if (net->weights == NULL || net->sources == NULL || net->dests == NULL)
    {
      printf
        ("Runtime error: Allocation failure in AddRandomizedConnections.\n");
      exit (1);
    }
  for (int from = fromstart; from <= fromend; from++)
    for (int to = tostart; to <= toend; to++)
...
struct nnet *
convertnetwork (struct _network *oldnet)
{
  struct nnet *newval;
  unsigned int layercount, neuroncount, connectioncount, nodenum;
  unsigned int synapsecount = 0;
  if (oldnet == NULL
      || NULL == (newval = (struct nnet *) calloc (1, sizeof (struct nnet))))
  ...
  for (layercount = 1; layercount < oldnet->num_of_layers; layercount++)        // for loop counts non-bias connections in old network
    for (neuroncount = 0;
         neuroncount < oldnet->layers[layercount].num_of_neurons;
         neuroncount++)
      newval->synapsecount +=
        oldnet->layers[layercount].neurons[neuroncount].num_input;
  newval->weights =
    (flotype *) malloc (newval->synapsecount * sizeof (flotype));
  newval->sources =
    (unsigned int *) malloc (newval->synapsecount * sizeof (unsigned int));
  newval->dests =
    (unsigned int *) malloc (newval->synapsecount * sizeof (unsigned int));
  newval->transfer =
    (enum activation_function *) malloc (sizeof (enum activation_function) *
                                         newval->nodecount);
  newval->accum =
    (enum accumulator_function *) malloc (sizeof (enum accumulator_function) *
                                          newval->nodecount);
  if (newval->weights == NULL || newval->sources == NULL
      || newval->dests == NULL || newval->transfer == NULL
      || newval->accum == NULL)
    {                           // if unable to allocate
      free (newval->weights);
      free (newval->sources);
      free (newval->weights);
      free (newval->transfer);
      free (newval->accum);
      free (newval);
      return (NULL);
    }
  for (layercount = 1; layercount < oldnet->num_of_layers; layercount++)
    {
      for (neuroncount = 0;
           ...
           neuroncount++)
        {
          ...
          newval->transfer[nodenum] =
            oldnet->layers[layercount].neurons[neuroncount].activation;
          newval->accum[nodenum] =
            oldnet->layers[layercount].neurons[neuroncount].accumulator;
          ...
          for (connectioncount = 0;
               ...
               connectioncount++)
            {
              ...
              newval->sources[synapsecount] =
                oldnet->layers[layercount].neurons[neuroncount].
                connection[connectioncount]->global_id + 1;
              newval->weights[synapsecount++] =
                (flotype) (oldnet->layers[layercount].neurons[neuroncount].
                           w[connectioncount]);
            }
        }
    }
  ...
  flotype wtmp;
  unsigned int srctmp;
  unsigned int desttmp;
  unsigned int skipsize;
  unsigned int i1;
  unsigned int i2;
  for (skipsize = (2 * --synapsecount) / 3; skipsize >= 1;
       skipsize = (2 * skipsize) / 3)
    for (connectioncount = 0; connectioncount < synapsecount;
         connectioncount++)
      {
        i1 =
          MIN (connectioncount, (connectioncount + skipsize) % synapsecount);
        i2 =
          MAX (connectioncount, (connectioncount + skipsize) % synapsecount);
        if (newval->sources[i1] > newval->sources[i2]
            || (newval->sources[i1] == newval->sources[i2]
                && newval->dests[i1] > newval->dests[i2]))
          {
            wtmp = newval->weights[i1];
            newval->weights[i1] = newval->weights[i2];
            newval->weights[i2] = wtmp;
            srctmp = newval->sources[i1];
            newval->sources[i1] = newval->sources[i2];
            newval->sources[i2] = srctmp;
            desttmp = newval->dests[i1];
            newval->dests[i1] = newval->dests[i2];
            newval->dests[i2] = desttmp;
          }
      }
  return (newval);
}
