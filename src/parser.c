 ...
 do{
  ...
  // syntax: TRAINING_POINT OUT point_index neuron_index value
   ...
   if(strcmp(s,"IN")==0){
    ...
    int conn=(int)(tmp);
    if(tmp!=conn){
     printf("TRAINING_POINT IN connection index must be an integer!\n");
     exit(0);
    }
    if(conn<0){
     printf("TRAINING_POINT IN connection index must be positive!\n");
     exit(0);
    }
    if(conn>(NEURON[neu].nw-1)){
     printf("TRAINING_POINT IN connection index out of range!\n");
     exit(0);
    }
    ret=fscanf(fp,"%lf",&tmp);
  ...
  // syntax: NETWORK_INPUT point_id neuron_id conn_id val
   ...
   int conn=(int)(tmp);
   if(tmp!=conn){
    printf("NETWORK_INPUT connection index must be an integer!\n");
    exit(0);
   }
   if(conn<0){
    printf("NETWORK_INPUT connection index must be positive!\n");
    exit(0);
   }
   if(conn>(NEURON[neu].nw-1)){
